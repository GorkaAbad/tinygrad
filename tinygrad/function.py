...
class Sin(Function):
    def forward(self, x: LazyBuffer) -> LazyBuffer:
        ...
        for n in range(1, N):
            term = term.e(BinaryOps.MUL, x_squared)
            factorial *= (2 * n) * (2 * n + 1)
            reciprocal_factorial = x.const(1.0 / factorial)
            current_term = term.e(BinaryOps.MUL, reciprocal_factorial)

            if sign == -1:
                result = result.e(BinaryOps.SUB, current_term)
            else:
    ...
    def backward(self, grad_output: LazyBuffer) -> LazyBuffer:
        cos_x = self.x.const(math.pi / 2).e(BinaryOps.SUB, self.x).e(UnaryOps.SIN)
        return cos_x.e(BinaryOps.MUL, grad_output)
...
class Sqrt(Function):
    ...
    def backward(self, grad_output: LazyBuffer) -> LazyBuffer:
        return grad_output.e(
            BinaryOps.MUL,
            self.ret.e(BinaryOps.MUL, self.ret.const(2)).e(UnaryOps.RECIP),
        )
...
class Div(Function):
    def forward(self, x: LazyBuffer, y: LazyBuffer) -> LazyBuffer:
        self.x, self.y = x, y
        return (
            x.e(BinaryOps.MUL, y.e(UnaryOps.RECIP))
            if not dtypes.is_int(x.dtype)
            else x.e(BinaryOps.IDIV, y)
        )
    ...
    def backward(
        ...
        return (
            grad_output.e(BinaryOps.MUL, self.y.e(UnaryOps.RECIP))
            if self.needs_input_grad[0]
            else None
        ), (
            grad_output.e(UnaryOps.NEG)
            .e(BinaryOps.MUL, self.x)
            .e(BinaryOps.MUL, self.y.e(BinaryOps.MUL, self.y).e(UnaryOps.RECIP))
...
class Max(Function):
    ...
    def backward(self, grad_output: LazyBuffer) -> LazyBuffer:
        ...
        max_is_1s = (
            ...
            .e(
                BinaryOps.SUB,
                self.x.e(BinaryOps.CMPNE, self.ret.expand(self.x.shape)).cast(
                    dtypes.float
                ),
            )
        )
        ...
        return (
            max_is_1s.e(BinaryOps.MUL, div.e(UnaryOps.RECIP))
            .cast(grad_output.dtype)
